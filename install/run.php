...
{
    return Cookie::read('U-CMS-install-timezone', 0) * 3600;
}
...
check(
    '<code>We were unable to run composer our selves. Please run "composer install" from the command line to install U-CMS. If you do not have composer installed please see <a href="https://getcomposer.org/">https://getcomposer.org/</a></code>',
    function () {
...
check(
    '<code>cms/config</code> directory needs to be temporarily writable
	so we can create your application and database configuration files.',
    function () {
        return is_writable(PATH . 'cms/config');
    }
);

check(
    'U-cms requires the php module <code>pdo_mysql</code> to be installed.',
    function () {
...
check(
    'U-cms requires the php module <code>GD</code> to be installed.',
    function () {
        return extension_loaded('gd');
    }
);

// mb_strtolower() in U-CMS\helpers.php
check(
    'U-CMS requires the php module <code>mbstring</code> to be installed.',
    function () {
