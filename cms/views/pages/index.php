...
<header class="wrap">
  <h1><?php echo __('pages.pages'); ?></h1>
    ...
    <?php if ($pages->count): ?>
      <nav>
          <?php echo Html::link('uladmin/pages/add', __('pages.create_page'), ['class' => 'btn']); ?>
          <?php echo Html::link('uladmin/menu', __('menu.edit_menu'), ['class' => 'btn']); ?>
      </nav>
    <?php endif; ?>
</header>
...
<section class="wrap">
  ...
  <nav class="sidebar statuses">
      <?php echo Html::link('uladmin/pages', '<span class="icon"></span> ' . __('global.all'), [
          'class' => ($status == 'all') ? 'active' : ''
      ]); ?>
      <?php foreach (['published', 'draft', 'archived'] as $type): ?>
          <?php echo Html::link('uladmin/pages/status/' . $type, '<span class="icon"></span> ' . __('global.' . $type),
              [
    ...
    <?php if ($pages->count): ?>
      <ul class="main list">
          <?php foreach ($pages->results as $item): $display_pages = array_merge([$item], $item->children()); ?>
              ...
              <li>
                <a href="<?php echo Uri::to('uladmin/pages/edit/' . $page->data['id']); ?>">
