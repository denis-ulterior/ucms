...
Route::collection(['before' => 'auth,csrf,install_exists'], function () {
     ...
     */
    Route::get([
        'uladmin/pages/status/(:any)',
        'uladmin/pages/status/(:any)/(:num)'
    ], function ($status, $page = 1) {
        $query   = Page::where('status', '=', $status);
        $perpage = Config::get('admin.posts_per_page');
        $total   = $query->count();
        $url     = Uri::to('uladmin/pages/status');
     ...
     * Edit Page
     */
    Route::get('uladmin/pages/edit/(:num)', function ($id) {
    ...
    Route::post('uladmin/pages/edit/(:num)', function ($id) {
        $input = Input::get([
            'parent',
            'name',
            'title',
    ...
    /*
        Add Page
    */
    Route::get('uladmin/pages/add', function () {
        $vars['token']     = Csrf::token();
    ...
    Route::post('uladmin/pages/add', function () {
        $input = Input::get([
            'parent',
            'name',
            'title',
        ...
        return Response::json([
            ...
            'redirect'     => Uri::to('uladmin/pages/edit/' . $id)
        ]);
    });
     ...
     * Delete Page
     */
    Route::get('uladmin/pages/delete/(:num)', function ($id) {
        if (
            (Page::count() > 1) &&
            (Page::home()->id != $id) &&
            (Page::posts()->id != $id) &&
        ...
        return Response::redirect('uladmin/pages');
    });
});
