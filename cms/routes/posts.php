...
Route::collection(['before' => 'auth,csrf,install_exists'], function () {
     ...
     */
    Route::get([
        'uladmin/posts/category/(:any)',
        'uladmin/posts/category/(:any)/(:num)'
        ...
        $perpage = Config::get('admin.posts_per_page');
        $total   = $query->count();
        $url     = Uri::to('uladmin/posts/category/' . $category->slug);
     ...
     */
    Route::get([
        'uladmin/posts/status/(:any)',
        'uladmin/posts/status/(:any)/(:num)'
    ], function ($status, $post = 1) {
        $query   = Post::where('status', '=', $status);
        $perpage = Config::get('admin.posts_per_page');
        $total   = $query->count();
        $url     = Uri::to('uladmin/posts/status/' . $status);
     ...
     * Edit post
     */
    Route::get('uladmin/posts/edit/(:num)', function ($id) {
    ...
    Route::post('uladmin/posts/edit/(:num)', function ($id) {
        $input = Input::get([
            'title',
            'slug',
            'description',
     ...
     * Add new post
     */
    Route::get('uladmin/posts/add', function () {
        $vars['token'] = Csrf::token();
        $vars['page']  = Registry::get('posts_page');
    ...
    Route::post('uladmin/posts/add', function () {
        $input = Input::get([
            'title',
            'slug',
            'description',
        ...
        if (Input::get('autosave') === 'true') {
            ...
            return Response::json([
                ...
                'redirect'     => Uri::to('uladmin/posts/edit/' . $id)
            ]);
        }
    });
     ...
     * Preview post
     */
    Route::post('uladmin/posts/preview', function () {
     ...
     * Delete post
     */
    Route::get('uladmin/posts/delete/(:num)', function ($id) {
        Post::find($id)->delete();
